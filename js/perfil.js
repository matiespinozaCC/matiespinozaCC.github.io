...
// Función para convertir pesos chilenos a dólares estadounidenses
function convertirPesosAUSD(pesos) {
  const tipoCambio = 796;
  const balanceUSD = balance / tipoCambio;
  return balanceUSD;
}
...
createOrder: function(data, actions) {
  const balanceUSD = convertirPesosAUSD(balance);
  return actions.order.create({
    purchase_units: [{
      amount: {
        currency_code: 'USD',
        value: balanceUSD.toFixed(2) // Redondea el valor a 2 decimales
      }
    }]
  });
},
